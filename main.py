...
def signal_handler(signum, frame):
    ...
    log_step("Received interrupt signal. Shutting down gracefully...", "warning")
    shutdown_flag = True
        ...
        try:
            ...
            task_data = {
                ...
                "task": {
                    ...
                    "websiteKey": CaptchaConfig.WEBSITE_KEY,
                    "pageAction": "login"
                }
            }
            
            log_step(f"Sending request to EzCaptcha: {task_data}", "info")
            response = self.session.post(create_task_url, json=task_data)
            response_text = response.text
            log_step(f"EzCaptcha response: {response_text}", "info")
            
            response.raise_for_status()
            result = response.json()
            
            # Check for error response
            if result.get("errorId", 1) != 0:  # errorId: 0 means success
                error_msg = result.get("errorDesc") or result.get("msg") or result.get("error") or "Unknown error"
                raise Exception(f"Failed to create task: {error_msg}")
                
            task_id = result.get("taskId")
            if not task_id:
                raise Exception(f"No taskId in response: {response_text}")
            ...
            for attempt in range(max_attempts):
                ...
                log_step(f"Task result attempt {attempt + 1}: {result}", "info")
                ...
                if result.get("status") == "ready":
                    ...
                    if token:
                        return token
                    raise Exception(f"No token in solution: {result}")
            ...
            log_step(f"Detailed error: {str(e)}", "error")
            raise Exception(f"EzCaptcha error: {str(e)}")

class CaptchaServiceFactory:
    @staticmethod
    def create_service(service_name: str, api_key: str):
        services = {
            "capmonster": ServiceCapmonster,
            "anticaptcha": ServiceAnticaptcha,
            "2captcha": Service2Captcha,
            "ezcaptcha": ServiceEzCaptcha
        }
...
async def main():
    try:
        ...
        ref_code = input(f"{Fore.GREEN}Enter referral code (example: 43HG9DLooqGcwKj): {Style.RESET_ALL}")
        ...
        print(f"3. 2Captcha")
        print(f"4. EzCaptcha{Style.RESET_ALL}")
        ...
        for i in range(num_referrals):
            if shutdown_flag:
                log_step("Gracefully stopping the process...", "warning")
                break
            ...
            try:
                ...
                log_step("Process interrupted by user", "warning")
                break
            except Exception as e:
        ...
        log_step(f"Unexpected error: {str(e)}", "error")
    finally:
        log_step("Cleanup complete", "info")

if __name__ == "__main__":
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        log_step("Program terminated by user", "warning")
    except Exception as e:
        log_step(f"Program error: {str(e)}", "error")
